    ...
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Version History') }} &mdash; {{ $userStory->title }}
            </h2>
            <a href="{{ route('user-stories.show', $userStory) }}" class="text-sm text-gray-500 hover:underline">
                &larr; {{ __('Back to user story') }}
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if ($versions->count() >= 2)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <form method="GET" action="{{ route('user-stories.versions.compare', $userStory) }}" class="flex items-end gap-4">
                        <div>
                ...
                <ul class="divide-y divide-gray-200">
                    @foreach ($versions as $version)
                        <li class="p-6 flex justify-between items-center">
                            <div>
                                <a href="{{ route('user-stories.versions.show', [$userStory, $version]) }}" class="font-medium text-gray-900 hover:underline">
                                    {{ __('Version') }} {{ $version->version_number }}
                                </a>
                                @if ($version->version_number === $userStory->current_version)
        ...
        @if ($version->version_number !== $userStory->current_version)
            @can('update', $userStory)
                <x-modal name="confirm-version-restore-{{ $version->id }}" focusable>
                    <form method="POST" action="{{ route('user-stories.versions.restore', [$userStory, $version]) }}" class="p-6">
                        @csrf
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            {{ __('This rewinds the user story back to this version\'s content. Nothing is deleted — the full version history stays intact.') }}
                        </p>
